...
class Discriminator(nn.Module):
    def __init__(self, num_dense = 16384):
        super().__init__()
        ...
        self.skip_first = nn.Conv1d(3, 128, 1)
        self.skip_sec = nn.Conv1d(128, 256, 1)
        self.skip_third = nn.Conv1d(256, 512, 1)
        self.skip_fourth = nn.Conv1d(512, 512, 1)
        self.dense = dense(512, 1)
        
    def forward(self, input):
        input = input.transpose(1,2)
        out_ns = self.first_conv(input)
        out_skip = self.skip_first(input)
        out = out_ns+out_skip

        out_ns = self.second_conv(out)
        out_skip = self.skip_sec(out)
        out = out_ns+out_skip

        out_ns =self.third_conv(out)
        out_skip = self.skip_third(out)
        out = out_ns + out_skip
